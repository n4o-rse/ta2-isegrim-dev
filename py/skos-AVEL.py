...
url = "https://docs.google.com/spreadsheets/d/1jEBkurkQzwT0_SsBGKPod--AQuS6IOs0/export?gid=1095998343&format=csv"

# read csv file
data = pd.read_csv(
    url,
    encoding='utf-8',
    usecols=['uuid', 'label', 'METAL', 'MetarialID', 'City', 'CityID'],
...
for index, row in data.iterrows():
    ...
    # typing
    lines.append("isegrim:type_" +
                 str(row['uuid']) + " " + "rdf:type" + " skos:Concept .")
    # lines.append("isegrim:type_" +
    #             str(row['uuid']) + " " + "rdf:type" + " prov:Entity .")

    # metadata
    lines.append("isegrim:type_" +
                 str(row['uuid']) + " " + "skos:prefLabel" + " '" + str(row['label']) + "'@en.")

    # skosplus items
    lines.append("isegrim:type_" + str(row['uuid']) + " " +
                 "skosplus:metal" + " '" + str(row['METAL']) + "'@en.")
    if str(row['MetarialID']) != 'nan':
        lines.append("isegrim:type_" + str(row['uuid']) + " " +
                     "skosplus:materialMatch isegrim:material_" + str(int(row['MetarialID'])) + ".")
    lines.append("isegrim:type_" + str(row['uuid']) + " " +
                 "skosplus:city" + " '" + str(row['City']) + "'@en.")
    if str(row['CityID']) != 'nan':
        lines.append("isegrim:type_" + str(row['uuid']) + " " +
                     "skosplus:cityMatch isegrim:city_" + str(int(row['CityID'])) + ".")
...
for x in range(1, int(files) + 1):
    strX = str(x)
    filename = Path.joinpath("rdf").joinpath("AV-EL.ttl")
