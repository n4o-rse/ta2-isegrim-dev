...
url = "https://docs.google.com/spreadsheets/d/1jEBkurkQzwT0_SsBGKPod--AQuS6IOs0/export?gid=2141343136&format=csv"

# read csv file
data = pd.read_csv(
    url,
    encoding='utf-8',
    usecols=['id', 'label', 'nomisma', 'wikidata', 'pleiades', 'gettytgn'],
...
for index, row in data.iterrows():
    ...
    if str(row['nomisma']) != 'nan':
        lines.append("isegrim:city_" + str(row['id']) + " " +
    ...
    if str(row['pleiades']) != 'nan':
        lines.append("isegrim:city_" + str(row['id']) + " " +
                     "skos:closeMatch" + " <http://pleiades.stoa.org/places/" + str(int(row['pleiades'])) + ">.")
    if str(row['gettytgn']) != 'nan':
        lines.append("isegrim:city_" + str(row['id']) + " " +
                     "skos:closeMatch" + " <http://vocab.getty.edu/tgn/" + str(int(row['gettytgn'])) + ">.")
...
for x in range(1, int(files) + 1):
    strX = str(x)
    filename = Path.joinpath("rdf").joinpath("city.ttl")
